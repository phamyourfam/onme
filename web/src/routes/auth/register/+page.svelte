<script lang="ts">
	...
	import { registerAction, getIsAuthenticated } from '$lib/stores/auth.svelte';
...
<svelte:head>
	<title>Create Account — OnMe</title>
</svelte:head>
...
<div class="flex min-h-screen items-center justify-center bg-surface-primary px-4">
	<div class="w-full max-w-sm">
		...
		<!-- Heading -->
		<h1 class="mb-8 text-2xl font-bold text-white">Create your account</h1>

		<!-- Form -->
		<form onsubmit={handleSubmit} class="flex flex-col gap-4">
			<div>
				<label for="register-email" class="mb-1 block text-sm text-pin-medium-gray">Email</label>
				<input
					id="register-email"
					type="email"
					bind:value={email}
			...
			<div>
				<label for="register-password" class="mb-1 block text-sm text-pin-medium-gray">Password</label>
				<input
			...
			<div>
				<label for="register-confirm" class="mb-1 block text-sm text-pin-medium-gray">Confirm password</label>
				<input
					id="register-confirm"
					type="password"
		...
		<p class="mt-6 text-center text-sm text-pin-medium-gray">
			...
			<a href="/auth/login" class="text-pin-red transition-colors hover:text-pin-red-hover">
				Log in
			</a>
		</p>
	</div>
